...
class TavilySearchWithImages(TavilySearchResults):  # type: ignore[override, override]
    ...
    api_wrapper: TavilySearchAPI = Field(
        default_factory=TavilySearchAPI
    ...
    def _process_results(
        ...
        cleaned_results = self.api_wrapper.clean_results_with_images(results)
        logger.debug(
    ...
    async def _arun(
        ...
        raw_results = await self.api_wrapper.async_raw_results(
            query,
            self.max_results,
            self.search_depth,
            self.include_domains,
